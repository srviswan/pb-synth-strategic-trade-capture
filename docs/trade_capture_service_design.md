# PB Synthetic Strategic Trade Capture Service Design

## Overview

The `pb-synth-strategic-trade-capture` project (PB Synthetic Strategic Trade Capture) processes incoming trade lots from upstream systems (front office trading and allocation management), applies economic and non-economic rules, performs validations and enrichments, and produces SwapBlotter output containing fully enriched contracts with EconomicTerms, PerformancePayout, and InterestPayout details.

Source repository: [https://github.com/srviswan/pb-synth-strategic-trade-capture](https://github.com/srviswan/pb-synth-strategic-trade-capture) — clone with `https://github.com/srviswan/pb-synth-strategic-trade-capture.git`
...
Paths are relative to the `pb-synth-strategic-trade-capture` repository root.

- **OpenAPI Specification**: `api/trade-capture-service-openapi.yaml`
- **Protobuf Schema**: `proto/trade_capture_message.proto`
- **Rule Configuration Schema**: `config/rule-configuration-schema.json` (schema definition for rule structure)
- **Rule Configuration Example**: `config/rules-configuration-example.yaml` (example rule structure)
